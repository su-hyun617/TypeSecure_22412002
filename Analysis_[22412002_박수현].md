...
1) textSource   
System이 시작되면 메인 화면 상단에 학습을 위해 입력할 문구가 출력된다.   
![그림1](images/textSource.png)

2) inputKey   
User 가 textSource에서 출력된 문구 입력 확인 및 backspace, del 키를 관리한다. Backspace 및 del 키의 경우 키들 사이의 시간 차이를 알기 어렵고 무의식적으로 발생하는 타이핑 리듬이 아닌 의식적인 행동의 결과이므로 결과에서 제거한다.   
![그림2](images/예외처리.png)

3) rhythmBuffer   
올바른 값이 입력되었음을 확인하기 전까지 입력된 데이터가 저장되어 있을 임시 저장소이다. 해당 입력이 올바르다고 인증될 경우 이는 로컬 저장소에 저장된다.   

4) typeCheck   
User가 입력한 문구와 textSource에서 출력된 문구가 동일한지 확인한다. 만약 동일하지 않을 경우 경고 문구를 팝업으로 띄운 뒤 rhythmBuffer에 임시 저장된 값을 discard한다.   
![그림3](images/잘못된 문구.png)

5) makePattern   
RhythmBuffer에서 받은 데이터를 리듬 배열로 변환한다.   

6) similarity   
인증 단계에서 user의 타이핑 데이터와 기존 타이핑 데이터 간의 유사도를 계산하고 이를 반환한다.   

7) patternUpdate   
textSource의 문구를 올바르게 입력했을 경우 혹은 인증 user임이 확인된 경우 현재의 타이핑 데이터를 기존의 데이터에 누적하여 저장한다.   
...
9) display   
인증하기 버튼 활성화, 텍스트 입력란 등 UI 전반을 다룬다.   
![그림5](images/UI1.png)
...
| Similarity | 저장된 타이핑 리듬과 현재 사용자의 타이핑 리듬 간의 유사도 |
| Buffer | 사용자의 타이핑 리듬을 저장하는 임시 보관소 |
...
## 6. References
https://news.sbs.co.kr/news/endPage.do?news_id=N1008109554
